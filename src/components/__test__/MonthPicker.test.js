...
describe('test MonthPicker component', () => {
    ...
    it('after click the button, dropdown should show, year list&month list should have the correct items', () => {
        ...
        expect(wrapper.find('.years-range .dropdown-item.active').text()).toEqual('2018 Year')
        expect(wrapper.find('.months-range .dropdown-item.active').text()).toEqual('08 Month')
        expect(wrapper.find('.years-range .dropdown-item').first().text()).toEqual(`${props.year - 4} Year`)
        expect(wrapper.find('.months-range .dropdown-item').first().text()).toEqual('01 Month')
      })
      it('click the year and month', ()=> {
        wrapper.find('.dropdown-toggle').simulate('click')
